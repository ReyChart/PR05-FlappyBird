  ...
  updata(bird, gameLoop, gameOver, score, medal) {
    ...
    this.pipesArray.forEach(pos => {
      ...
      (bird.birdPosition.x + bird.birdRadius > pos.x && bird.birdPosition.x - bird.birdRadius < pos.x + this.pipeWidth && bird.birdPosition.y + bird.birdRadius > this.bottomPipeYPos && bird.birdPosition.y - bird.birdRadius < this.bottomPipeYPos + this.pipeHeight) || bird.birdPosition.y + bird.birdSize[1] / 2 >= this.canvas.element.height - this.canvas.foreground.height) {
        ...
        // ЗАКОММЕНТИРОВАННЫЙ ВАРИАНТ ПОЗВОЛЯЕТ НАЙТИ КНОПКУ "START" В СПРАЙТШИТЕ
        // ДОБАВИТЬ КНОПКЕ ОБРАБОТКУ КЛИКА, А ТАКЖЕ КУРСОР "POINTER"
        // ВАРИАНТ ХОРОШИЙ, НО ЕСЛИ ИГРАТЬ С ТЕЛЕФОНА, ТО МЕНЯЕТСЯ РАЗРЕШЕНИЕ
        // СООТВЕТСТВЕННО И КООРДИНАТЫ НУЖНО ПЕРЕОПРЕДЕЛЯТЬ

        // document.addEventListener('click', (event) => {
        //   this.clickX = event.clientX - this.startBtnCoords.left;
        //   this.clickY = event.clientY - this.startBtnCoords.top;

        //   if (this.clickX > this.startBtn.x && this.clickX < this.startBtn.x + this.startBtn.width && this.clickY > this.startBtn.y && this.clickY < this.startBtn.y + this.startBtn.height) {
        //     this.gameReset.restart();
        //   }
        // });

        // document.addEventListener('mousemove', (event) => {
        //   this.cursorX = event.clientX - this.startBtnCoords.left;
        //   this.cursorY = event.clientY - this.startBtnCoords.top;

        //   if (this.cursorX >= this.startBtn.x && this.cursorX <= this.startBtn.x + this.startBtn.width && this.cursorY >= this.startBtn.y && this.cursorY <= this.startBtn.y + this.startBtn.height) {
